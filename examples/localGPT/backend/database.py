...
class ChatDatabase:
    ...
    def delete_session(self, session_id: str) -> bool:
        ...
        with self._db() as conn:
            # Remove session-index links first to satisfy FK constraints
    ...
    def index_name_exists(self, name: str) -> bool:
        """Check if an index with the given name already exists."""
        with self._db() as conn:
            cursor = conn.execute('SELECT COUNT(*) FROM indexes WHERE name = ?', (name,))
