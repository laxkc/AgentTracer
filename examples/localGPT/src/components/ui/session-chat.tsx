    ...
    try {
      ...
      try {
        ...
        if (idxResp.indexes && idxResp.indexes.length > 0) {
          ...
          setIsIndexed(true)  // Session has indexes, mark as indexed
          setUploadedFiles([]) // Clear any uploaded files state
        }
      } catch {}
    } catch (error) {
      ...
      {showIndexForm && (
        <IndexForm
          ...
          onIndexed={(s)=>{
            ...
            setCurrentSession(s);
            setIsIndexed(true);  // Mark as indexed so chat isn't blocked
            setUploadedFiles([]); // Clear any uploaded files state
