  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('already exists')) {
        alert(`❌ An index named "${indexName}" already exists.\n\nPlease choose a different name or delete the existing index first.`);
      } else if (errorMessage.includes('Create index error')) {
        // Extract the actual error from the message
        const match = errorMessage.match(/Create index error: (.+)/);
        const actualError = match ? match[1] : errorMessage;
        alert(`❌ Failed to create index: ${actualError}`);
      } else {
